...
def parse_args():
    ...
    args.add_argument("-oc1", "--out_channels1", type=int, default=500,
                      help="Number of output channels in conv layer")
    args.add_argument("-oc2", "--out_channels2", type=int, default=500,
                      help="Number of output channels in conv layer")
...
def train_conv(args):
    ...
    model_conv = SpKBGATConvOnly(entity_embeddings, relation_embeddings, args.entity_out_dim, args.entity_out_dim,
                                 ...
                                 args.nheads_GAT, args.out_channels1, args.out_channels2)
    ...
    print('Conv model entity embeddings: ', model_conv.final_entity_embeddings.size())
    print('Conv model relation embeddings: ', model_conv.final_relation_embeddings.size()) 
    ...
    margin_loss = torch.nn.Softplus() #SoftMarginLoss
    ...
    for epoch in range(args.epochs_conv):
        ...
        for iters in range(num_iters_per_epoch):
            ...
            if CUDA:
                train_indices = Variable(
                    torch.LongTensor(train_indices)).cuda()
                train_values = Variable(torch.FloatTensor(train_values),requires_grad=False).cuda()

            else:
                train_indices = Variable(torch.LongTensor(train_indices))
                train_values = Variable(torch.FloatTensor(train_values))

            logits, conv_input_ht, conv_input_ht_prime = model_conv(args.valid_invalid_ratio_conv, Corpus_, Corpus_.train_adj_matrix, train_indices)

            optimizer.zero_grad()

            loss = margin_loss(-train_values.data * logits).mean() + mse_loss(conv_input_ht, conv_input_ht_prime).mean()
...
def evaluate_conv(args, unique_entities):
    ...
    model_conv.eval()
    with torch.no_grad():
        valid_mean_rank, valid_mean_recip_rank, valid_hits_one, valid_hits_three, valid_hits_ten = Corpus_.get_validation_pred(args, model_conv, unique_entities)  # valid result
        test_mean_rank, test_mean_recip_rank, test_hits_one, test_hits_three, test_hits_ten = Corpus_.get_testing_pred(args, model_conv, unique_entities)  # test result
        print("\nValid:")
        print("valid_MR: ",valid_mean_rank)
        print("valid_MRR: ",valid_mean_recip_rank)
        print("valid_Hits@1: ",valid_hits_one)
        print("valid_Hits@3: ",valid_hits_three)
        print("valid_Hits@10: ",valid_hits_ten)
